...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == "register") { // rejestracja
$db = new mysqli("localhost", "root", "", "auth");

$email = $_REQUEST['email'];
$email = filter_var($email, FILTER_SANITIZE_EMAIL); // mail ma byc poprawny

$password = $_REQUEST['password'];
$passwordRepeat = $_REQUEST['passwordRepeat']; // hasło pobieranie

$imie = $_REQUEST['imie'];
$nazwisko = $_REQUEST['nazwisko']; // pobieranko imie i nazwisko

if($password == $passwordRepeat) { // hasła identico = go next
$passwordHASH = password_hash($password, PASSWORD_ARGON2I);
$q = $db->prepare("INSERT INTO user VALUES (NULL, ?, ?, ?, ?");
$q->bind_param("ss", $email, $passwordHASH, $imie, $nazwisko);
...
echo "coś sie zesrało"; } }
else {    
echo "naucz sie pisać"; }
}
